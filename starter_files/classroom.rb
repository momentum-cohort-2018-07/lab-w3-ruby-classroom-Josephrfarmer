...
def assignment_average_score(data, assignment)
    scores = assignment_scores(data, assignment)
...
def averages(data)
    data.transform_values do |scores|
        scores.sum/scores.length
    end
end
...
def final_letter_grades(data)
    averages(data).transform_values do |scores|
        letter_grade(scores)
    end
end
